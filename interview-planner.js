...
function createInterviewPlan({ profile, job, role, analysis }) {
  ...
  const analyzedRequirements = (analysis?.requirements || []).filter(item => ['required', 'preferred'].includes(item.classification)).sort((left, right) => (right.importance ?? -1) - (left.importance ?? -1));
  const requirements = analyzedRequirements.length ? analyzedRequirements.map(item => item.skill) : job ? requirementsFor(job) : [...(profile.skills || []), ...(profile.keywords || [])].slice(0, 6);
  const topics = requirements.map(skill => { const judgment = analyzedRequirements.find(item => item.skill === skill); return { topic: skill, status: hasSkill(profile, skill) ? 'known area' : 'development suggestion', priority: hasSkill(profile, skill) ? 'practice' : 'study first', reason: judgment ? `${judgment.classification === 'required' ? 'Required' : 'Preferred'} in the local source analysis${judgment.importance === null ? '' : `; priority ${judgment.importance}/3`}.` : job ? `Listed in the selected ${job.title} description.` : `Relevant to your confirmed ${selectedRole} target.` }; });
  ...
  return { summary: `Written preparation guidance for ${selectedRole}, based on confirmed skills and ${analyzedRequirements.length ? 'source-backed requirement priorities' : 'the selected job description'}.`, selectedRole, selectedJob: job ? { id: job.id, title: job.title, company: job.company } : null, topics, studyPlan: focus.length ? focus.map((topic, index) => `${index + 1}. Study ${topic.topic}: ${topic.reason}`) : ['1. Practice explaining your confirmed strengths with concrete, truthful examples.'], questionBank, generatedBy: analyzedRequirements.length ? 'local source analysis and deterministic planner' : 'deterministic local planner' };
}
